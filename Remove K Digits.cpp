...
public:
    string removeKdigits(string num, int k) {
        // number of operations greater than the length we return an empty string
        if(num.length() <= k)   
            return "0";
        
        // k is 0, no need to remove/perform any operation
        if(k == 0)
            return num;
        ...
        for(int i = 1; i<num.length(); ++i)
        {
            ...
            {
                //If k is greater than 0 our stack is not empty and the upcoming digit,
